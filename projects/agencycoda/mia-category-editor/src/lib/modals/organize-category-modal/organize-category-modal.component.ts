...
export class OrganizeCategoryModalComponent implements OnInit {
  ...
  onClickNewCategory() {
    ...
    if(this.config.idCategoryDefault){
      ob = this.categoryModal.openWithType(new MiaCategory(), this.config.idCategoryDefault);
    } else {
  ...
  loadItems(items: Array<any>) {
    ...
    .pipe(tap(res => this.categories.map(c => this.dropListIds.push('drop-category-' + c.id))))
    .pipe(map(res => this.categories.map(c => c.items = items.filter(i => i[this.config.fieldCategoryId!] == c.id))))
